...
- **Same-tier downgrades** — optionally considers a smaller release with the same quality label, such as replacing a 20 GB Bluray‑1080p file with a 4 GB Bluray‑1080p release. Requires the replacement be smaller by at least a configurable percentage (default 50%) — two same-tier releases are often nearly the same size, so "smaller" alone isn't enough to call it a real downgrade.
